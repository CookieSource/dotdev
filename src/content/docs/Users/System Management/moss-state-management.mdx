...
<Aside type="caution">
    The system model is a fundemental change in how the system is managed. Please keep this in mind and carefully read the instructions below.
</Aside>

The system-model is an opt in feature on AerynOS that allows us to use a .KDL file to declare the repositories and packages that should be present on our system.
Once we switch to the system model if we want to install new packages we will have to modify the system-model.kdl file in /usr/? we then need to sync our system we can do so by running the sync command.

```bash
sudo moss sync -u
```
This will check the system-model file for changes and apply those changes.
...
sudo moss 
```
The opt-in can also be undone by removing the KDL file.
...
<br></br><br></br>
We can both **export** and **import** a system model file. This means you can export your current system configuration and someone else can import that configuration to get the same installed packages and repositories. 
### Export a system-model file
```bash
sudo moss 
```

### Import a system-model file
