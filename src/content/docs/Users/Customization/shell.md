...
If Bash is missing, install it with moss:

```
sudo moss install bash
```
...
Shell performance is mostly about startup time and script execution speed. Lightweight shells (such
as dash or mksh when used for `/bin/sh`) tend to start faster for scripting workloads, while feature-
rich shells (such as Zsh or Fish) trade some startup time for interactive features. AerynOS tracking
notes and early shellbench results for mksh are discussed here:
https://github.com/orgs/AerynOS/discussions/72#discussioncomment-14310151
