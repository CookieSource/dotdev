---
title: KDE Wallet
summary: A quick introduction to using KWallet on AerynOS
---
...
Out of the box, the AerynOS KDE Plasma Desktop is already configured to use the KDE Wallet PAM module, which unlocks the KDE Wallet on session login.
...
On the AerynOS KDE Plasma Desktop spin, `ksshaskpass` is installed out of the box and `SSH_ASKPASS` is set to `ksshaskpass` in the file `/usr/share/xdg/plasma-workspace/env/50-aerynos-defaults.sh` by default.
