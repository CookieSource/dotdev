---
title: PostgreSQL
summary: A quick guide to using Postgres on AerynOS
---

# PostgreSQL server

[PostgreSQL](https://www.postgresql.org/) is an open source object-relational database system. The `postgresql` package in AerynOS includes the PostgreSQL library, client and server.
...
Because PostgreSQL is included as a single package, the only way to perform a major upgrade of PostgreSQL is via a backup/restore. The procedure below describes the upgrade process as it applies to AerynOS. See the [PostgreSQL documentation](https://www.postgresql.org/docs/18/upgrading.html#UPGRADING-VIA-PGDUMPALL) for more details.

1.  If you have already updated PostgreSQL, install the 17.6 builds from your repository snapshot:

```bash
sudo moss install postgresql postgresql-libpq
```
