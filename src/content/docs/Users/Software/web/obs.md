...
summary: A quick start guide to OBS on AerynOS
---
...
```bash
sudo moss install obs-studio
```

## Virtual camera

OBS includes a virtual webcam device which can be used to send your OBS stream to video conferencing programs. In order to use this on AerynOS, you must first install from our repository a separate kernel module that is responsible for creating the necessary "loopback" device. There are two versions of this software available in the repository, you must choose the version that corresponds to the kernel you have installed.
...
- If you are running the "current" kernel, install `v4l2loopback-current`: `sudo moss install v4l2loopback-current`
- If you are running the "lts" kernel, install `v4l2loopback`: `sudo moss install v4l2loopback`
...
In essence, you should have `bin/64bit` folders inside the plugin folder, with any shared object plugin files inside that. As OBS Studio for AerynOS only supports 64-bit, the plugin **must** support 64-bit as well.
...
```bash
sudo moss install libuiohook unzip
```
