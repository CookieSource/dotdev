...
To recover your system, you need to mount the AerynOS root (`/`) partition and all other partitions your system uses.

1. Open a terminal.
1. Mount the AerynOS root partition:
   ...
   2. Create a directory to serve as the mount point for your AerynOS system:

      ```bash
      mkdir /target
      ```

   3. Find the drive and partition of your AerynOS system using the `lsblk` command.
      ...
      └─sdb2                  8:18   0 464.8G  0 part
        ├─AerynOSSystem-root  253:0    0 234.3G  0 lvm
        ├─AerynOSSystem-swap  253:1    0   3.7G  0 lvm  [SWAP]
      ...
      You can use drive size or the number of partitions in each drive to determine which one is your AerynOS drive.

      :::tip

      If you installed AerynOS on a _SATA_ drive, the name of the drive might be `sdX` (For example, `sda`). If you installed AerynOS on an _NVMe_ drive, the name of the drive might be `nvme#n#` (For example `nvme0n1`).
   ...
   4. If the type of your drive is **lvm** or if your drive is encrypted:
      ...
      1. Check if the drive is encrypted.
         ...
         ├─sdb1                  8:17   0     1G  0 part
         └─sdb2                  8:18   0 464.8G  0 part
           ├─AerynOSSystem-root  253:0    0   100G  0 lvm
           └─AerynOSSystem-swap  253:1    0     8G  0 lvm  [SWAP]
         ```
   ...
   1. Mount the AerynOS root partition:
      ...
      - If your drive's type is _lvm_
        ```bash
        mount /dev/mapper/AerynOSSystem-Root /target
        ```
      - Otherwise:
        ```bash
...
1. Chroot to your AerynOS system:
